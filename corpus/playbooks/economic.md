# Economic Disruption — Supply-Chain Disruption Response Standard

**Document ID:** SCR-STD-ECON-001  |  **Version:** 2.7  |  **Status:** Controlled  |  **Owner:** Supply Chain Resilience Office
...
This standard applies when the active disruption is an economic event. Where more than one type of disruption is active simultaneously, this type is addressed at priority 4 in the order geopolitical, natural disaster, cyber, economic, labour. The assessment runs in two passes, each working through provisions in order; the first provision whose conditions are fully satisfied takes effect and ends further checking within that pass.
...
### (ECON-NULL) Disruption outside network boundary
...
### (ECON-DT) Deep-tier structural fallthrough — no Tier-1 supplier scored
...
### (ECON-DT-NULL) Deep-tier with no structural concern
...
### (ECON-R3) Shared sub-tier convergence override
...
### (ECON-DEP) Medium-to-high-risk supplier with high dependency concentration
...
### (ECON-R1) High-risk Tier-1 supplier
...
### (ECON-MEDS) Medium-risk supplier flagged by the disruption-type priority signal
...
### (ECON-LOWS) Low-risk supplier flagged by the disruption-type priority signal
...
### (ECON-LOW) Low-risk routine supplier — catch-all
...
The standing posture for economic disruptions is **Cost Containment**. Diffuse and cost-pressured: hold efficient posture but protect structurally concentrated dependencies (Tomlin 2006).
...
**EX-ECON-DEP** (illustrates provision ECON-DEP). Scenario: sub_dependency_ratio: high; count_disrupted_paths: 4. The correct response is to **replace**, escalating to **Executive Leadership**.