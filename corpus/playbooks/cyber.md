# Cyber Disruption — Supply-Chain Disruption Response Standard

**Document ID:** SCR-STD-CYB-001  |  **Version:** 2.7  |  **Status:** Controlled  |  **Owner:** Supply Chain Resilience Office
...
This standard applies when the active disruption is a cyber event. Where more than one type of disruption is active simultaneously, this type is addressed at priority 3 in the order geopolitical, natural disaster, cyber, economic, labour. The assessment runs in two passes, each working through provisions in order; the first provision whose conditions are fully satisfied takes effect and ends further checking within that pass.
...
### (CYB-NULL) Disruption outside network boundary
...
### (CYB-DT) Deep-tier structural fallthrough — no Tier-1 supplier scored
...
### (CYB-DT-NULL) Deep-tier with no structural concern
...
### (CYB-R3) Shared sub-tier convergence override
...
### (CYB-HUB) High-centrality hub supplier
...
### (CYB-R1) High-risk Tier-1 supplier
...
### (CYB-MEDS) Medium-risk supplier flagged by the disruption-type priority signal
...
### (CYB-LOWS) Low-risk supplier flagged by the disruption-type priority signal
...
### (CYB-LOW) Low-risk routine supplier — catch-all
...
The standing posture for cyber disruptions is **Rapid Reconfiguration**. Fast, propagating, non-spatial: contain by rapid isolation / rerouting (Tomlin 2006) before the ripple spreads (Dolgui et al. 2018).
...
**EX-CYB-HUB** (illustrates provision CYB-HUB). Scenario: sub_node_centrality: high. The correct response is to **replace**, escalating to **Executive Leadership**.