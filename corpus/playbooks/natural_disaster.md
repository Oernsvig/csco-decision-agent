# Natural-Disaster Disruption — Supply-Chain Disruption Response Standard

**Document ID:** SCR-STD-NAT-001  |  **Version:** 2.7  |  **Status:** Controlled  |  **Owner:** Supply Chain Resilience Office
...
This standard applies when the active disruption is a natural-disaster event. Where more than one type of disruption is active simultaneously, this type is addressed at priority 2 in the order geopolitical, natural disaster, cyber, economic, labour. The assessment runs in two passes, each working through provisions in order; the first provision whose conditions are fully satisfied takes effect and ends further checking within that pass.
...
### (NAT-NULL) Disruption outside network boundary
...
### (NAT-DT) Deep-tier structural fallthrough — no Tier-1 supplier scored
...
### (NAT-DT-NULL) Deep-tier with no structural concern
...
### (NAT-R3) Shared sub-tier convergence override
...
### (NAT-R1) High-risk Tier-1 directly in the disrupted region
...
### (NAT-R5) Medium-risk in-region supplier with high downstream exposure breadth
...
### (NAT-R2) High-risk Tier-1 outside the disrupted region
...
### (NAT-MEDS) Medium-risk supplier flagged by the disruption-type priority signal
...
### (NAT-LOWS) Low-risk supplier flagged by the disruption-type priority signal
...
### (NAT-LOW) Low-risk routine supplier — catch-all
...
The standing posture for natural disaster disruptions is **Rapid Reconfiguration**. Acute, shorter disruptions favour flexibility / contingent rerouting over standing redundancy (Tomlin 2006).
...
**EX-NAT-05** (illustrates provision NAT-R5). Scenario: tier1_in_region: True; sub_exposure_breadth: high. The correct response is to **replace**, escalating to **Executive Leadership**.